...
def prepare_graph(g, features, n_layers, n_hidden):
    ...
    * Performs first aggretgation step ndata['preprocess']
    ...
    g.ndata['norm'] = norm
    print("Done")
    print("First round of feature aggregation")  # TODO FIXME this actually breaks subgraphing
    ...
    for i in range(n_layers):
        g.ndata['h_{}'.format(i)] = torch.zeros(features.shape[0], n_hidden)

    g.ndata['h_{}'.format(n_layers-1)] = torch.zeros(features.shape[0], 2*n_hidden)
    print("Done")


def train(model, optimizer, loss_fcn, num_neighbors, g, train_nid, labels, epochs,
          batch_size=32, num_workers=4):
    device = model.layers[0].linear.weight.device
    ...
    for epoch in range(epochs):
        ...
        for nf in dgl.contrib.sampling.NeighborSampler(g, batch_size,
            ...
            for i, names in enumerate(node_embed_names):
                for name in names:
                    nf.layers[i].data[name] = nf.layers[i].data[name].to(device)

            model.train()
            # forward
            pred = model(nf)
            batch_nids = nf.layer_parent_nid(-1).to(device=pred.device).long()
            batch_labels = labels[batch_nids].to(pred.device)
            ...
            for i, names in enumerate(node_embed_names):
                for name in names:
                    nf.layers[i].data[name] = nf.layers[i].data[name].to(g.ndata[name].device)
            nf.copy_to_parent(node_embed_names=node_embed_names)

        print("[" + str(epoch+1) + "]", loss_acc / steps)

def copy_params(infer_model, model):
    for infer_param, param in zip(infer_model.parameters(), model.parameters()):
        infer_param.data.copy_(param.data)


def evaluate(infer_model, g, test_nid, labels, batch_size=32, num_workers=10):
    num_acc = 0.
    n_layers = infer_model.n_layers

    n_test_samples = test_nid.size(0)
    for nf in dgl.contrib.sampling.NeighborSampler(g, batch_size,
                                                   g.number_of_nodes(),
                                                   neighbor_type='in',
                                                   num_workers=32,
